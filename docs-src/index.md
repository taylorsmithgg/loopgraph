...
Everything here is deterministic: SQLite, subprocesses, exit codes. No model is
in the path of any decision the gates make. The one place a second model does
appear — [auditing a check for spec-gaming](audit.md) — runs at authoring time,
gates nothing, and hands its verdict to a human.
...
`add` refuses a check that already passes: one that is green before the work
cannot tell done from not-done. Among checks that do discriminate, the widest
one wins — see [weakness, not brevity](gates.html#weakness-not-brevity).

## What it is not

- **Not a planner.** It holds the definition of done; deciding how to get there
  is the agent's job.
- **Not a judge.** No model scores anything. Measured judges over-reject
  conformant work 35–45%, which is why nothing here gates on one.
- **Not a hosted service.** One SQLite file per repository, on your machine.

## Hooks

Six hooks, all off by default and all failing open — a broken gate allows work
rather than blocking it. See [Gates](gates.md) for what each one does and the
[design](design.md) for why the loop gate needs `spec_prompt.py` to be
reachable at all.
