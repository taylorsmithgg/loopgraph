...
def main() -> int:
    ...
    try:
        ...
        # A refusal is an explicit, recorded decision that a criterion cannot
        # be met from here -- blocked on a production action that is somebody
        # else's, or on access this session does not have. It must stop the
        # gate driving at it, or the loop wedges for its whole block budget on
        # work no amount of effort here can finish. It must NOT make the
        # criterion disappear: that is how an open compromise becomes invisible
        # again. So: named on the way past, not driven at.
        refused = [c for c in crits if coord.refusal(conn, c["id"])]
        refused_ids = {c["id"] for c in refused}
        crits = [c for c in crits if c["id"] not in refused_ids]
        ...
        for c in refused:
            r = coord.refusal(conn, c["id"]) or {}
            loose.append({"id": c["id"],
                          "statement": c["statement"],
                          "why": "REFUSED: " + str(r.get("reason", ""))[:120]})
